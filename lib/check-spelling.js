        ...
        dictToUse((err, dict) => {
            ...
            const terms = searchTerm.split(' ').map((word) => {
                ...
                const suggestions = isCorrect
                    ? []
                    : spell.suggest(wordAlphaNumeric);
            ...
            function buildSuggestions() {
                const permutationLimit = max(
                    terms.map((term) => term.suggestions.length)
                );
                const permutations = new Array(permutationLimit).fill(
                    searchTerm
                );
                const termsWithSuggestions = terms.filter(
                    (term) => term.suggestions.length > 0
                );
                return permutations.map((permutation, index) => {
                    termsWithSuggestions.forEach((term) => {
                        const replacement =
                            term.suggestions[index] || term.suggestions[0];
